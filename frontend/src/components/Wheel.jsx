...
const Wheel = ({ position, possibleValues, turnCount }) => {
  ...
  const text = values.map((value, i) => {
    ...
    const characters = `$${value.toString()}`.split("").map((char, i) => {
      const currFontSize = fontSize * (1 - 0.15 * i);
      return (
        <div
          key={i}
          style={{
    ...
    <div
      style={{
        ...
        boxShadow: `${outsideShadow}px ${outsideShadow}px ${
          ...
          insideShadow * 3
        }px inset ${theme.shadowDark}, -${insideShadow}px -${insideShadow}px ${
          insideShadow * 3
